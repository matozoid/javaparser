...
public class YamlPrinter {
    ...
    public void output(Node node, String name, int level, StringBuilder builder) {
        ...
        if (outputNodeType)
            builder.append(System.lineSeparator() + indent(level) + name + "(Type=" + metaModel.getTypeName() + "): "+": "+node.getTokenRange().get() +"   "+node.getRange().get());
        else
            builder.append(System.lineSeparator() + indent(level) + name + ": "+node.getTokenRange().get());
